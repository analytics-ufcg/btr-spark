...
```{r, warning=FALSE, message=FALSE}
summarised.dates <- busData %>% group_by(date) %>%
  summarise(meanCrowdedness = mean(probableNumPassengers),
            medianCrowdedness = median(probableNumPassengers))

temp1 <- busData %>% select(date, hourOrig, hourDest, probableNumPassengers)
...
```{r, warning=FALSE, message=FALSE}
summarised.weeks <- busData %>% group_by(weekOfYear) %>%
  summarise(medianWeek = median(probableNumPassengers))

temp3 <- busData %>% select(weekOfYear, weekDay, probableNumPassengers)
...
```{r, warning=F, message=F}
hour.median.num.passengers <- busData %>% 
  group_by(hourOrig) %>% 
  summarise(
    median.num.passengers = median(probableNumPassengers)
  )
...
hour.median.num.passengers <- NULL
bus.data.grid <- NULL
...
```{r, warning=F, message=F}
ggplot(busData, aes(x = duration, y = probableNumPassengers)) +
  geom_point() +
...
```{r, warning=F, message=F}
duration.percentiles <- quantile(busData$duration, c(.05, .95), na.rm = TRUE)
passengers.percentiles <- quantile(busData$probableNumPassengers, c(.05, .95), na.rm = TRUE)
