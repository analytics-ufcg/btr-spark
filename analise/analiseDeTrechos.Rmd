...
```{r, warning=F, message=F}
dados.bulma <- busData %>% select(route, busStopIdOrig, hourOrig, date, hourDest, weekDay, weekOfYear, shapeLatOrig, shapeLonOrig, duration, problem)
rm(busData)
...
summarised.dates <- dados.bulma %>% group_by(date) %>%
  summarise(meanDuration = mean(duration, na.rm = T),
            medianDuration = median(duration, na.rm = T))
...
summarised.weeks <- dados.bulma %>% group_by(weekOfYear) %>%
  summarise(medianWeek = median(duration, na.rm = T))
...
temp4 <- temp3 %>% group_by(weekDay) %>%
  summarise(
   medianOfDiffs = median(diffFromMedian, na.rm = T)
)
rm(temp3)
...
dados.bulma.grid %>%
  ...
  summarise(median.duration = median(duration, na.rm = T)) %>% 
...
summarised.grid <- dados.bulma.grid %>% 
  ...
  summarise(
    median.duration = median(duration, na.rm = T)
  )
...
rm(hour.median.duration)
rm(dados.bulma.grid)

hourly.summarised.joined.grid <- joined.grid %>% 
  group_by(plotting.lat, plotting.lon) %>% 
  summarise(
    median.diff.duration = median(diff.duration, na.rm = T)
  )
rm(joined.grid)
